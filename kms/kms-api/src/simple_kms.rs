// Simplified KMS Service without complex module dependencies
use crate::types::*;
...
impl KmsService {
    ...
    pub async fn create_key(&self, request: CreateKeyRequest) -> Result<CreateKeyResponse> {
        ...
        let private_key_bytes = secret_key.secret_bytes().to_vec();
    ...
    pub async fn sign(&self, request: SignRequest) -> Result<SignResponse> {
        ...
        let secp = Secp256k1::new();
        let secret_key = SecretKey::from_slice(&stored_key.private_key)
        ...
        // Ensure hash is 32 bytes for secp256k1
        let hash_32 = if hash.len() >= 32 {
            &hash[0..32]
        } else {
            return Err(anyhow!("Hash too short for secp256k1"));
        };

        // Create ECDSA signature
        let message = secp256k1::Message::from_slice(hash_32)
