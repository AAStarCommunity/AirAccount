...
2. 装 `finalize-helper.sh` → `/opt/aastar/node-setup/`（0755 root:root）+ `install -m440 aastar-node-setup.sudoers /etc/sudoers.d/aastar-node-setup`（`visudo -c` 校验）
   - ⚠️ **脚本 root:root 还不够,父目录链也必须 root 拥有、`aastar-setup` 不可写**（`/opt`、`/opt/aastar`、`/opt/aastar/node-setup` 均 root:root ≤0755）。否则目录可写 = 攻击者删掉脚本重建一个恶意同名文件,借 NOPASSWD 白名单以 root 跑 —— sudoers 脚本白名单的经典逃逸口。
