...
| 5 | `kms/docs/api/openapi.yaml` | `version:`（约 line 8）+ 头注释（line 2）**+ ⚠️ 本版新增/改动的端点必须补进 `paths:`（不是只 bump 版本号——v0.22.0 曾漏掉 `/attestation` + `/.well-known/*`，事后补）**。🔒 **`version:` 必须 == `KMS_VERSION`（行4），由 `scripts/ca-ta-consistency.py` 机械门 assert（0.29.0 时漏 bump→SDK 检测不到，CC-106 加门）——漏了 CI 直接红** | OpenAPI |
