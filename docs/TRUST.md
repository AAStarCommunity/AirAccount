...
**铸造（mint）= 用户在场（强）+ nonce 绑定**
- 铸造 `create-agent-key` / `create-p256-session-key` **必须带 WebAuthn 断言，且由 TA 在 TEE 内独立校验**用户在场（#111 / C-1）——被攻陷的 host 也无法无用户铸出凭证。这是 mint 的关键防线。
- challenge 绑定到 TA 一次性 per-wallet nonce（防重放）。
- **关于 mint 参数绑定（#115 已回退）**：mint 的参数（`agent_index`/`session_index` 服务端原子分配、`subject`/`ttl` host 派生）**都不是客户端授权/可知的值**——客户端只发 `human_key_id + label`，strict 下客户端算不出「参数 commitment」（不可满足）。已回退为 nonce-only；真正有意义的加固应绑**客户端授权的输入**（`human_key_id + label`，需把 label 接进 TA），列为 follow-up。
