...
17. [Dev Wallet — 开发测试凭证](#17-dev-wallet--开发测试凭证)
...
```json
{
  "Passkey": {
    ...
    "ClientDataHash": "<hex-string-32bytes>",
    "Signature": "<hex-string-64bytes-r||s>"
  }
}
```
...
| 格式 | 长度 | 说明 |
|------|------|------|
| Raw r\|\|s | **64 bytes** (128 hex chars) | **推荐**。r (32 bytes big-endian) 拼接 s (32 bytes big-endian) |
| DER | 70-72 bytes | ECDSA DER 编码。CA 自动检测并解析 |

Hex 前缀 `0x` 可选，有无均可。

示例（Raw r||s 格式）：
```
Signature: "e4da0f14429a47d4c165c11e3509dc7a2e9f18964351591d1b68f2c749620a4e256bb1ed856bb53ee6dbde6f3d4dd73d5fb68d43f47da6ba8c2e26215521b197"
            |<---------- r (32 bytes) ---------->||<---------- s (32 bytes) ---------->|
```

**验证流程**：CA 将 `AuthenticatorData || ClientDataHash` 拼接后做 SHA-256，用钱包绑定的 P-256 公钥验证 Signature。
...
### 5.6 POST /DeriveAddress

派生以太坊地址（BIP-32 HD 密钥派生）。需要 PassKey 认证。

**Headers**: `x-amz-target: TrentService.DeriveAddress`, `x-api-key`

**Request**:
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| `Address` | string | 模式1 | 以太坊地址（自动查 KeyId + Path） |
| `KeyId` | string | 模式2 | 钱包 UUID |
| `DerivationPath` | string | 模式2 | HD 路径 (如 `m/44'/60'/0'/0/1`) |
...
```json
{
  "Address": "0xf1024bbf5544f2eee444a347c38baa11330c43b5",
  "PublicKey": "[PUBKEY_FROM_TA]"
}
```

**三种地址解析方式**:
1. `Address` → 从 DB 查 KeyId + Path（直接返回已缓存地址）
2. `KeyId` + `DerivationPath` → 调用 TA 派生新地址
3. `KeyId` only → 使用钱包默认 Path

**DerivationPath 格式**: `m/44'/60'/0'/0/N`，N = 0-9999。CA 端做格式验证，不合规则拒绝。

**性能**: ~1.16s（含 p256-m verify ~320ms）
...
```json
{
  ...
  "Options": {
    "rp": { "name": "AirAccount KMS", "id": "aastar.io" },  // 或 "localhost"（取决于请求 Origin）
    "user": {
      "id": "<base64url>",
      "name": "wallet-user",
...
- **rpId 自动解析**: 服务端从 HTTP `Origin` header 匹配 `KMS_RP_ID` 列表。例如 `Origin: http://localhost:5173` → `rpId: "localhost"`，`Origin: https://app.aastar.io` → `rpId: "aastar.io"`。无 Origin 则用列表第一个（默认 `aastar.io`）
- **Origin 安全**: 未配置的域名（如 `https://evil.com`）可获取 challenge，但 CompleteRegistration 时 `KMS_ORIGIN` 检查会拒绝（origin mismatch 错误）
...
| 字段 | JSON 类型 | 必填 | 说明 |
|------|-----------|------|------|
| `chainId` | number | Yes | EIP-155 chain ID (e.g. 1=mainnet, 11155111=sepolia) |
| `nonce` | number | Yes | 交易 nonce |
| `to` | string | Yes | 目标地址 hex `"0x..."` |
| `value` | **string** | Yes | 转账金额 **hex string** (e.g. `"0xde0b6b3a7640000"` = 1 ETH) |
| `gasPrice` | **string** | Yes | Gas 价格 **hex string** (e.g. `"0x4a817c800"` = 20 Gwei) |
| `gas` | number | Yes | Gas limit (e.g. 21000) |
| `data` | string | Yes | Calldata hex，无数据传 `""` |

> **注意**: `value` 和 `gasPrice` 是 **hex 字符串**（不是数字），`gas` 是数字。
...
返回 KMS 版本号。可用于验证部署的 TA/CA 是否是预期版本。
...
`version` 是 KMS API 版本，`build` 是 Cargo.toml 中的 package version。两者一致说明部署正确。
...
```json
{
  ...
  "ClientDataHash": "<hex-32bytes>",
  "Signature": "<hex-64bytes-r||s>"
}
```

| 字段 | 长度 | 说明 |
|------|------|------|
| `AuthenticatorData` | 变长 (≥37 bytes) | WebAuthn authenticatorData hex。含 rpIdHash(32) + flags(1) + signCount(4) + 可选扩展 |
| `ClientDataHash` | 32 bytes (64 hex chars) | SHA-256(clientDataJSON) 的 hex 编码 |
| `Signature` | 64 bytes (128 hex chars) | **推荐 Raw r\|\|s 格式**：r(32 bytes) 拼接 s(32 bytes)，big-endian。也支持 DER (70-72 bytes)，CA 自动检测 |

Hex 前缀 `0x` 可选。

**CA 验证算法**：
1. `msg = AuthenticatorData_bytes || ClientDataHash_bytes`
2. `hash = SHA-256(msg)`
3. `P-256_ECDSA_Verify(wallet.passkey_pubkey, hash, Signature)` → pass/fail

**两种认证方式的选择**：`Passkey` 和 `WebAuthn` 二选一。如果两者都传，优先使用 `WebAuthn`。都不传则返回 400 错误。
...
| `KMS_RP_ID` | `aastar.io` | WebAuthn Relying Party ID（逗号分隔多值，如 `aastar.io,localhost`）|
| `KMS_RP_NAME` | `AirAccount KMS` | WebAuthn Relying Party 显示名 |
| `KMS_ORIGIN` | `https://{KMS_RP_ID[0]}` | WebAuthn 允许的 Origin（逗号分隔，支持通配符 `*.aastar.io`）|
| `KMS_RATE_LIMIT` | `100` | 每 API Key 每分钟最大请求数 |
...
## 17. Dev Wallet — 开发测试凭证

以下凭证已在 DK2 生产板上预创建，可直接用于 API 测试。

### 17.1 钱包信息

| 项 | 值 |
|----|---|
| Wallet ID | `c9558d8b-9cb6-432a-aed0-bb7c3a548589` |
| Address | `0x6cce5babf8afa2936b291368389ab06e4efb4151` |
| Public Key | `0x03d736f337e399bbf3333582a11e3b817a92ac01fc26d841493a79399ae0e79e10` |
| Private Key | `0xe90dad6b89f4d0a091cdbd87db6e663e85066ebcd7f838d831717fc05ecd1ca5` |
...
| 项 | 值 |
|----|---|
| Public Key (uncompressed) | `0x04da9589a2a2742d35e49569bcc033a9d796cc9b3089f8093a51eb8776bfd259f3ac839769be270d3d41986774671372637016414cf0531c3ab2f74dbc5fe42421` |
| Private Key | `0x183890a797c54a8aa7bde79949817d4e26f3ab937ca042ac789b4a4f76ca122d` |

### 17.3 API Key

```
kms_b3994135cfd148ec9c5be29ef0690679
```

### 17.4 快速验证

```bash
HOST="192.168.7.2:3000"    # 本地 DK2 直连
# HOST="kms1.aastar.io"    # 生产环境 (通过 Cloudflare)
API_KEY="kms_b3994135cfd148ec9c5be29ef0690679"
KEY_ID="c9558d8b-9cb6-432a-aed0-bb7c3a548589"

# 1. 健康检查 + 版本验证
curl -s http://$HOST/version
# {"version":"0.15.22","build":"0.15.22"}

# 2. 查询钱包
curl -s -X POST http://$HOST/DescribeKey \
  -H "Content-Type: application/json" \
...
# 3. 列出所有钱包
curl -s -X POST http://$HOST/ListKeys \
  -H "Content-Type: application/json" \
...
签名操作需要实时生成 P-256 ECDSA assertion。使用 `kms/test/p256_helper.py`：

```bash
cd kms/test
PASSKEY_PRIV="0x183890a797c54a8aa7bde79949817d4e26f3ab937ca042ac789b4a4f76ca122d"

# 生成 assertion（每次签名需要新的 assertion）
ASSERTION=$(python3 p256_helper.py assertion "$PASSKEY_PRIV")
...
# SignHash
curl -s -X POST http://$HOST/SignHash \
  -H "Content-Type: application/json" \
  -H "x-amz-target: TrentService.SignHash" \
  -H "x-api-key: $API_KEY" \
  -d "{\"KeyId\":\"$KEY_ID\",\"Hash\":\"0xa1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2\",\"Passkey\":{\"AuthenticatorData\":\"$AUTH_DATA\",\"ClientDataHash\":\"$CDH\",\"Signature\":\"$SIG\"}}"
```

> **注意**: 每次签名操作都需要生成新的 assertion（新的 authenticator_data + client_data_hash + signature）。assertion 不可重用。

---

## 18. End-to-End Examples

### 18.1 Legacy 流程（不用 WebAuthn Ceremony）

适用于已有 P-256 密钥对的服务端集成。

```bash
HOST="192.168.7.2:3000"
API_KEY="kms_b3994135cfd148ec9c5be29ef0690679"

# ── Step 1: 生成 P-256 密钥对（一次性） ──
cd kms/test
python3 p256_helper.py gen
# 输出: { "private_key_pem": "...", "public_key_hex": "04...", ... }
# 保存 private_key_pem 到安全位置，public_key_hex 用于创建钱包

PUBKEY="04da9589a2a2742d35..."  # 你的 P-256 公钥

# ── Step 2: 创建钱包 ──
curl -s -X POST http://$HOST/CreateKey \
  -H "Content-Type: application/json" \
  -H "x-amz-target: TrentService.CreateKey" \
  -H "x-api-key: $API_KEY" \
  -d "{\"Description\":\"my-wallet\",\"KeyUsage\":\"SIGN_VERIFY\",\"KeySpec\":\"ECC_SECG_P256K1\",\"Origin\":\"EXTERNAL_KMS\",\"PasskeyPublicKey\":\"$PUBKEY\"}"
# → {"KeyMetadata":{"KeyId":"<uuid>", ...}}

KEY_ID="<上一步返回的 KeyId>"

# ── Step 3: 等待地址派生 (~90s 首次) ──
while true; do
    STATUS=$(curl -s "http://$HOST/KeyStatus?KeyId=$KEY_ID" | python3 -c "import sys,json; print(json.load(sys.stdin)['Status'])")
    echo "Status: $STATUS"
    [ "$STATUS" = "ready" ] && break
    sleep 3
done
...
# ── Step 4: 签名 ──
# 生成新 assertion
ASSERTION=$(python3 p256_helper.py assertion "<your-private-key-pem>")
AUTH_DATA=$(echo "$ASSERTION" | python3 -c "import sys,json; print(json.load(sys.stdin)['authenticator_data'])")
CDH=$(echo "$ASSERTION" | python3 -c "import sys,json; print(json.load(sys.stdin)['client_data_hash'])")
SIG_R=$(echo "$ASSERTION" | python3 -c "import sys,json; print(json.load(sys.stdin)['signature_r'])")
SIG_S=$(echo "$ASSERTION" | python3 -c "import sys,json; print(json.load(sys.stdin)['signature_s'])")
...
curl -s -X POST http://$HOST/SignHash \
  ...
  -H "x-amz-target: TrentService.SignHash" \
  -H "x-api-key: $API_KEY" \
  -d "{\"KeyId\":\"$KEY_ID\",\"Hash\":\"0x<your-32byte-hash>\",\"Passkey\":{\"AuthenticatorData\":\"$AUTH_DATA\",\"ClientDataHash\":\"$CDH\",\"Signature\":\"$SIG\"}}"
# → {"Signature":"<secp256k1-signature-hex>"}

# Sign Transaction
ASSERTION=$(python3 p256_helper.py assertion "<your-private-key-pem>")
# ... 同上提取字段 ...
curl -s -X POST http://$HOST/Sign \
  -H "Content-Type: application/json" \
  -H "x-amz-target: TrentService.Sign" \
  -H "x-api-key: $API_KEY" \
  -d "{\"KeyId\":\"$KEY_ID\",\"DerivationPath\":\"m/44'/60'/0'/0/0\",\"Transaction\":{\"chainId\":1,\"nonce\":0,\"to\":\"0x742d35Cc6634C0532925a3b844Bc9e7595f2bD18\",\"value\":\"0xde0b6b3a7640000\",\"gasPrice\":\"0x4a817c800\",\"gas\":21000,\"data\":\"\"},\"Passkey\":{\"AuthenticatorData\":\"$AUTH_DATA\",\"ClientDataHash\":\"$CDH\",\"Signature\":\"$SIG\"}}"
# → {"Signature":"f86c80850...","TransactionHash":"..."}

# ── Step 5: 删除钱包 ──
ASSERTION=$(python3 p256_helper.py assertion "<your-private-key-pem>")
# ... 同上提取字段 ...
curl -s -X POST http://$HOST/DeleteKey \
  -H "Content-Type: application/json" \
  -H "x-amz-target: TrentService.ScheduleKeyDeletion" \
  -H "x-api-key: $API_KEY" \
  -d "{\"KeyId\":\"$KEY_ID\",\"Passkey\":{\"AuthenticatorData\":\"$AUTH_DATA\",\"ClientDataHash\":\"$CDH\",\"Signature\":\"$SIG\"}}"
# → {"KeyId":"...","DeletionDate":"..."}
```

### 18.2 WebAuthn Ceremony 流程

适用于浏览器/App 集成，使用 [SimpleWebAuthn](https://simplewebauthn.dev/) 兼容协议。

```
1. POST /BeginRegistration      → 获取 Options
2. navigator.credentials.create(Options)  → 获取 Credential
3. POST /CompleteRegistration    → 创建钱包，返回 KeyId
4. GET  /KeyStatus?KeyId=xxx    → 轮询直到 "ready"
5. POST /BeginAuthentication     → 获取 challenge
6. navigator.credentials.get(Options)     → 获取 assertion
7. POST /SignHash (with WebAuthn field)   → 签名
```

**浏览器端代码示例** (使用 @simplewebauthn/browser)：

```javascript
import { startRegistration, startAuthentication } from '@simplewebauthn/browser';

const API = 'https://kms1.aastar.io';
const headers = { 'Content-Type': 'application/json', 'x-api-key': 'kms_...' };

// ── 注册 ──
const regBegin = await fetch(`${API}/BeginRegistration`, {
  method: 'POST', headers,
  body: JSON.stringify({ Description: 'my-wallet', UserName: 'user@example.com' })
}).then(r => r.json());

const credential = await startRegistration(regBegin.Options);

const regComplete = await fetch(`${API}/CompleteRegistration`, {
  method: 'POST', headers,
  body: JSON.stringify({ ChallengeId: regBegin.ChallengeId, Credential: credential })
}).then(r => r.json());

const keyId = regComplete.KeyId;
// 轮询 /KeyStatus 直到 ready ...

// ── 签名 ──
const authBegin = await fetch(`${API}/BeginAuthentication`, {
  method: 'POST', headers,
  body: JSON.stringify({ KeyId: keyId })
}).then(r => r.json());

const assertion = await startAuthentication(authBegin.Options);

const signResult = await fetch(`${API}/SignHash`, {
  method: 'POST', headers: { ...headers, 'x-amz-target': 'TrentService.SignHash' },
  body: JSON.stringify({
    KeyId: keyId,
    Hash: '0xa1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2c3d4e5f6a1b2',
    WebAuthn: { ChallengeId: authBegin.ChallengeId, Credential: assertion }
  })
}).then(r => r.json());
