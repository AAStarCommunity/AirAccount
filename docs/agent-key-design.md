...
KMS 返回  = 0x08 ‖ account(20) ‖ key(20) ‖ R(32) ‖ S(32) ‖ V(1)  // 106 bytes（v0.17.2）
```

**`/kms/sign-agent` 请求字段：**
- `payload` = 裸 `userOpHash`（32 字节 hex），EIP-191 包裹在 TEE 内做
- `accountAddress`（v0.17.2 新增）= Smart Account 合约地址（20 字节 hex）

**签名字节布局（v0.17.2，106 字节）：**

| 字节范围 | 内容 | 来源 |
|---------|------|------|
| [0] | `0x08`（SessionKeyValidator 类型标识） | 固定 |
| [1:21] | `account`（Smart Account 地址） | 请求方传入，合约验证 |
| [21:41] | `key`（agent secp256k1 EOA 地址） | TEE 内从私钥推导 |
| [41:73] | ECDSA `r` | TEE 内签名 |
| [73:105] | ECDSA `s` | TEE 内签名 |
| [105] | ECDSA `v`（27 or 28） | TEE 内签名 |

链上验证：`SessionKeyValidator.validateUserOp` 从 sig[1:21] 取 account，
从 sig[21:41] 取 key，用 `toEthSignedMessageHash(userOpHash)` recover，
验证 `recovered == key` 且 `account == userOp.sender`。

> **迁移说明**：v0.17.0 为 `[0x08][ECDSA(65)]` = 66 字节，SDK 外部拼装。
> v0.17.2 起 KMS 直接返回 106 字节，SDK 无需额外处理。
