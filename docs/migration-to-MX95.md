...
> **当前实际起点**：AirAccount KMS 开发环境已是 **aarch64 QEMU**（`aarch64-unknown-linux-gnu`），与 i.MX93 / i.MX95 指令集完全相同，**无需 target triple 变更或 32-bit 适配**。
> 本文档保留存储架构对比（DK2 REE-FS vs RPMB）部分。完整 MX95 迁移方案见 [`hardware-imx95-fetmx9596c.md`](hardware-imx95-fetmx9596c.md)。
