...
mod wallet_storage {
    ...
    // OP-TEE环境下的安全存储 - P0安全修复
    // 注意：OP-TEE TA 通常是单线程运行，所以我们使用简化的安全存储
    ...
    fn get_wallet_storage() -> &'static mut WalletStorage {
        unsafe {
            if WALLET_STORAGE.is_none() {
                WALLET_STORAGE = Some(WalletStorage::new());
            }
            WALLET_STORAGE.as_mut().unwrap()
        }
    }
    ...
    pub fn create_wallet() -> Option<WalletId> {
        ...
        if let Some(slot) = storage.wallets.iter_mut().find(|w| w.is_none()) {
            let id = storage.next_wallet_id;
            storage.next_wallet_id += 1;
    ...
    pub fn remove_wallet(id: WalletId) -> bool {
        let storage = get_wallet_storage();
        if let Some(slot) = storage.wallets.iter_mut()
    ...
    pub fn list_wallets() -> ([Option<WalletId>; MAX_WALLETS], usize) {
        ...
        let storage = get_wallet_storage();
        for (i, wallet_opt) in storage.wallets.iter().enumerate() {
...
fn get_security_manager() -> &'static mut SecurityManager {
    unsafe {
        if SECURITY_MANAGER.is_none() {
            SECURITY_MANAGER = Some(SecurityManager::default());
        }
        SECURITY_MANAGER.as_mut().unwrap()
    }
}
...
mod input_validation {
    ...
    pub fn validate_command_parameters(cmd_id: u32, params: &mut Parameters) -> Result<(), ValidationError> {
        ...
        match cmd_id {
            CMD_ECHO => {
                // Echo命令需要输入和输出缓冲区
                if let (Ok(mut p0), Ok(mut p1)) = (unsafe { params.0.as_memref() }, unsafe { params.1.as_memref() }) {
...
// P0安全修复：混合熵源处理函数
use crate::hybrid_entropy_ta::get_hybrid_entropy_ta;
...
fn handle_create_hybrid_account(input_buffer: &[u8], output_buffer: &mut [u8]) -> Result<usize, &'static str> {
    ...
    // 获取混合熵源TA实例
    let security_manager = get_security_manager();
    let entropy_ta = get_hybrid_entropy_ta(security_manager);
    
    // 初始化如果需要
    if let Err(_) = entropy_ta.initialize() {
        return Err("Failed to initialize hybrid entropy system");
    }
    
    // 在TEE内派生账户密钥
    let account_key = match entropy_ta.derive_user_account_key(user_email, passkey_public_key) {
        Ok(key) => key,
...
fn handle_sign_with_hybrid_key(input_buffer: &[u8], output_buffer: &mut [u8]) -> Result<usize, &'static str> {
    ...
    // 使用测试密钥进行签名（实际中应该根据账户ID重新派生）
    let test_account_key = [0x42u8; 32]; // 测试密钥
    let signature = match entropy_ta.sign_transaction_secure(&test_account_key, transaction_hash) {
        Ok(sig) => sig,
        Err(_) => return Err("Failed to sign transaction"),
    };
...
fn handle_verify_security_state(output_buffer: &mut [u8]) -> Result<usize, &'static str> {
    ...
    // 验证安全状态
    let security_verified = match entropy_ta.verify_security_state() {
    ...
    let response = if security_verified {
        ...
        b"security_state:FAILED,tee_entropy:FAIL,memory_protection:UNKNOWN"
    };
