...
The AirAccount KMS is a TEE-based key management system with an AWS KMS compatible API. It features a layered architecture:

1.  **Client Layer**: SDKs (JS, Python, Rust), CLI tools, and web apps.
2.  **API Gateway Layer**: Cloudflare Tunnel for secure HTTPS proxying and load balancing.
3.  **KMS Service Layer**: An Axum-based Rust server (`kms-api`) that handles AWS KMS API compatibility.
4.  **Core Logic Layer**: A hardware-independent Rust crate (`kms-core`) for cryptography and key management.
5.  **TEE Layer**: A `kms-host` adapter communicates with the Trusted Application (`kms-ta`) running in the secure environment.

## Development Roadmap

The project is developed in phases. The current focus is **Phase 8: Security Enhancement and Real TEE Deployment**.

*   **Phase 7 (Completed)**: Established a Mock-TEE environment, AWS KMS compatible API, and a full testing suite.
*   **Phase 8 (Current)**: Migrating from Mock-TEE to a real QEMU OP-TEE environment, enhancing security, and preparing for production deployment.
*   **Phase 9 (Future)**: Advanced key management features (e.g., rotation), multi-chain support, and developer ecosystem building.
*   **Phase 10 (Long-term)**: Production hardware deployment (Raspberry Pi 5), enterprise features, and industry standard certifications.

## Building and Running

The project's initial phase focuses on setting up a local development environment using QEMU on macOS.
...
The project follows a staged testing strategy:

*   **Stage 1: API Design Validation**: Uses a Mock TA to verify the KMS API mapping without a full TA implementation.
*   **Stage 2: Integration Testing**: Validates the end-to-end flow with a real `eth_wallet` TA in the OP-TEE QEMU environment.
*   **Stage 3: Production-Ready Testing**: Verifies deployment and operation on real hardware (Raspberry Pi).

## Development Conventions

*   The project follows a phased development roadmap, starting with a simulated environment (QEMU) and progressing to hardware integration.
