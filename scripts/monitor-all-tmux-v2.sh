#!/bin/bash
# 使用 tmux 在单个窗口中启动所有监控 (V2 - 改进版)
# 使用更稳定的监控脚本避免 socat 问题
...
# 面板 1 (右上): CA (使用替代脚本 - 更稳定)
tmux send-keys -t $SESSION:0.1 "cd $(pwd) && ./scripts/monitor-terminal2-ca-alt.sh" C-m

# 面板 2 (左下): TA (使用替代脚本 - 更稳定)
tmux send-keys -t $SESSION:0.2 "cd $(pwd) && ./scripts/monitor-terminal3-ta-alt.sh" C-m
...
tmux select-pane -t $SESSION:0.1 -T "CA-API"
...
echo ""
echo "⚡ 改进说明："
echo "   - Terminal 2 使用更稳定的监控方案"
echo "   - 避免 socat 在 tmux 中的不稳定问题"
echo "   - 从 Cloudflared 日志提取 API 调用信息"
echo ""
echo "📺 进入监控界面："
