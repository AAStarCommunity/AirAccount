#!/bin/bash
# KMS API Server 简化测试脚本 - 在QEMU Guest VM中运行（无需jq）

echo "🎯 KMS API Server 功能测试"
echo "================================"

# 检查并启动API服务器
if ! ps | grep -v grep | grep -q kms-api-server; then
    echo "⚠️  API服务器未运行，正在启动..."
    ./kms-api-server > /tmp/kms-api-server.log 2>&1 &
    sleep 3

    # 检查健康端点
    if curl -s http://localhost:3000/health > /dev/null 2>&1; then
        echo "✅ API服务器启动成功"
    else
        echo "❌ API服务器启动失败"
        exit 1
    fi
else
    echo "✅ API服务器已运行"
fi

echo ""
echo "================================"
echo ""

# 1. 创建密钥
echo "1️⃣ 测试 CreateKey..."
CREATE_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.CreateKey' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d '{
    "Description": "Test Key for QEMU",
    "KeyUsage": "SIGN_VERIFY",
    "KeySpec": "ECC_SECG_P256K1",
    "Origin": "AWS_KMS"
  }')

echo "$CREATE_RESPONSE"
echo ""

# 提取KeyId（简单字符串处理）
KEY_ID=$(echo "$CREATE_RESPONSE" | grep -o '"KeyId":"[^"]*"' | head -1 | sed 's/"KeyId":"\([^"]*\)"/\1/')

if [ -n "$KEY_ID" ]; then
    echo "✅ 密钥创建成功: $KEY_ID"
else
    echo "❌ CreateKey 失败"
    exit 1
fi

# 2. 获取公钥
echo ""
echo "2️⃣ 测试 GetPublicKey..."
GET_PUB_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.GetPublicKey' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d "{\"KeyId\": \"$KEY_ID\"}")

echo "$GET_PUB_RESPONSE"
echo ""

if echo "$GET_PUB_RESPONSE" | grep -q "PublicKey"; then
    echo "✅ GetPublicKey 成功"
else
    echo "⚠️  GetPublicKey 返回空（可能未实现）"
fi

# 3. 派生地址
echo ""
echo "3️⃣ 测试 DeriveAddress..."
DERIVE_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.DeriveAddress' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d "{\"KeyId\": \"$KEY_ID\", \"DerivationPath\": \"m/44'/60'/0'/0/0\"}")

echo "$DERIVE_RESPONSE"
echo ""

if echo "$DERIVE_RESPONSE" | grep -q "Address"; then
    ADDRESS=$(echo "$DERIVE_RESPONSE" | grep -o '"Address":"[^"]*"' | sed 's/"Address":"\([^"]*\)"/\1/')
    echo "✅ DeriveAddress 成功: $ADDRESS"
else
    echo "❌ DeriveAddress 失败"
fi

# 4. 签名交易
echo ""
echo "4️⃣ 测试 Sign (签名以太坊交易)..."
SIGN_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.Sign' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d "{
    \"KeyId\": \"$KEY_ID\",
    \"DerivationPath\": \"m/44'/60'/0'/0/0\",
    \"Transaction\": {
      \"chainId\": 1,
      \"nonce\": 0,
      \"to\": \"0x742d35Cc6634C0532925a3b844Bc454e4438f44e\",
      \"value\": \"0x0de0b6b3a7640000\",
      \"gasPrice\": \"0x04a817c800\",
      \"gas\": 21000,
      \"data\": \"0x\"
    }
  }")

echo "$SIGN_RESPONSE"
echo ""

if echo "$SIGN_RESPONSE" | grep -q "Signature"; then
    echo "✅ Sign 成功"
else
    echo "❌ Sign 失败"
fi

# 5. 列出密钥
echo ""
echo "5️⃣ 测试 ListKeys..."
LIST_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.ListKeys' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d '{}')

echo "$LIST_RESPONSE"
echo ""

if echo "$LIST_RESPONSE" | grep -q "Keys"; then
    echo "✅ ListKeys 成功"
else
    echo "⚠️  ListKeys 返回空列表"
fi

# 6. 描述密钥
echo ""
echo "6️⃣ 测试 DescribeKey..."
DESCRIBE_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.DescribeKey' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d "{\"KeyId\": \"$KEY_ID\"}")

echo "$DESCRIBE_RESPONSE"
echo ""

if echo "$DESCRIBE_RESPONSE" | grep -q "KeyState"; then
    echo "✅ DescribeKey 成功"
else
    echo "❌ DescribeKey 失败"
fi

# 7. 删除密钥
echo ""
echo "7️⃣ 测试 ScheduleKeyDeletion..."
DELETE_RESPONSE=$(curl -s -X POST http://localhost:3000/ \
  -H 'X-Amz-Target: TrentService.ScheduleKeyDeletion' \
  -H 'Content-Type: application/x-amz-json-1.1' \
  -d "{\"KeyId\": \"$KEY_ID\", \"PendingWindowInDays\": 7}")

echo "$DELETE_RESPONSE"
echo ""

if echo "$DELETE_RESPONSE" | grep -q "DeletionDate"; then
    echo "✅ ScheduleKeyDeletion 成功"
else
    echo "❌ ScheduleKeyDeletion 失败"
fi

# 测试总结
echo ""
echo "================================"
echo "🎉 KMS API 测试完成！"
echo ""
echo "API服务器日志: /tmp/kms-api-server.log"
echo "查看日志: tail -f /tmp/kms-api-server.log"
echo ""
echo "停止API服务器: killall kms-api-server"